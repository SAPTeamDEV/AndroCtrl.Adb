...
namespace SAPTeam.AndroCtrl.Adb.Tests
{
    public class FramebufferHeaderTests
    {
        ...
        {
            byte[] data = File.ReadAllBytes("framebufferheader.bin");
        ...
        {
            byte[] data = File.ReadAllBytes("framebufferheader-v2.bin");
        ...
        public void ToImageTest()
        {
            ...
            byte[] framebuffer = File.ReadAllBytes("framebuffer.bin");
        ...
        {
            byte[] data = File.ReadAllBytes("framebufferheader-empty.bin");
            FramebufferHeader header = FramebufferHeader.Read(data);
